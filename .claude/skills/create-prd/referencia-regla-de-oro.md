...
📝 ¿Y esos # y ? Son Markdown: el # marca la jerarquía de títulos y los ponen negrita. Si no es lo tuyo, no te enganches —podés escribir tu PRD en Google Docs o Word, lo que importa es el contenido—. Si querés repasarlo, lo vemos paso a paso en Markdown y editar archivos.
...
💡 ¿Y la IA? Hoy lo armás a mano para entrenar el ojo: entender qué mira cada chequeo. Pero la IA entra enseguida. En el Módulo 2, vibecodeando, vas a volver sobre este PRD y mejorarlo —al construir descubrís huecos y lo refinás—; y en el Módulo 4 vas a tener skills como create-prd y validate-prd que lo generan y validan automáticamente contra este mismo checklist. Para entonces vas a saber leer si la IA hizo un buen trabajo o te metió humo.
